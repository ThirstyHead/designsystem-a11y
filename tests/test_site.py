"""Structural + accessibility + tone validator for the designsystem-a11y site.
...
PAGES = ["index.html", "colors.html", "spacing.html", "typography.html"]
ORIGIN = "https://thirstyhead.com/designsystem-a11y"
...
def test_no_forbidden_elements(page):
    ...
    # Allow <script> for theme toggle logic
    allowed = {"script"}
...
def _resolve_tokens(theme: str) -> dict[str, str]:
    css = (SITE / "css" / "tokens.css").read_text(encoding="utf-8")
    
    # Extract all variables from the whole file first
    all_vars = {}
    # Find all --name: value;
    ...
    # If dark theme, we need to ensure the dark overrides take precedence.
    # The current regex finds all, and the last one wins. 
    # Since [data-theme="dark"] comes after :root, the dark values are already in all_vars.
    # However, to be explicit and handle themes correctly:
    
    if theme == "light":
        # For light theme, we only want the :root values, NOT the [data-theme="dark"] ones.
...
def test_text_contrast_aa(theme):
    ...
    pairs = [
        ...
        ("--sys-color-accent-primary", "--sys-color-bg-primary"),
    ]
    for fg, bg in pairs:
